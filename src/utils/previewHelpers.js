...
export function generatePreviewRows(results) {
  const { totalFlour, totalSalt, totalMalt, bigaYeast, refreshYeast, totalYeastPercent, bigaYeastPercent, refreshYeastPercent } = results;
  ...
  return [
    ...
    {
      label: "Yeast",
      biga: `${formatGrams(bigaYeast, "yeast")} (${formatBakersPercent(bigaYeastPercent)})`,
      dough: `${formatGrams(refreshYeast, "yeast")} (${formatBakersPercent(refreshYeastPercent)})`,
